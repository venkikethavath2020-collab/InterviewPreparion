...
## 1. Overview
- **What this file does**: Variable declaration semantics (`var`, `let`, `const`), scope, hoisting, and mutability contracts.
...
## 2. Core Concept
- **Main concept**: Variable declaration semantics (`var`, `let`, `const`), scope, hoisting, and mutability contracts.
- **Internal working**: JavaScript allocates bindings in lexical environments; `var` is function-scoped and hoisted differently from block-scoped `let`/`const`.
...
- **Design pattern used**: Foundational state declaration and environment setup.
...
No explicit reusable functions are defined; the file is primarily conceptual/demo-driven.

## 5. Interview Focus
- **What interviewers expect**: Interviewers test scope rules, temporal dead zone, and reassignment constraints.
- **Common questions**:
...
- `let` is block-scoped; avoid `var` in modern code.
- Undeclared assignment leaks globals (non-strict mode).
- Uninitialized `let` defaults to `undefined` after declaration.
- Use explicit declarations for maintainability.
