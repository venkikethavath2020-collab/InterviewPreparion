...
## 1. Overview
- **What this file does**: Hoisting behavior for declarations and initialization timing (especially TDZ for `let`/`const`).
...
## 2. Core Concept
- **Main concept**: Hoisting behavior for declarations and initialization timing (especially TDZ for `let`/`const`).
...
## 5. Interview Focus
- **What interviewers expect**: Interviewers expect exact execution-order explanation, not vague definitions.
- **Common questions**:
